...
bool		cfg_list(Stream *s, char **args)
{
  ...
  s->println("KEY:\tVALUE:");
  for (p = CanRemoteCfg.startNode(); p; p = p->next())
    {
      s->print(p->key());
      s->print(":\t");
      s->println(p->value());
...
bool		cfg_get(Stream *s, char **args)
{
  ...
  res =CanRemoteCfg.getValue(args[2]); 
  if (res == "")
    {
      s->print(args[2]);
...
bool		cfg_set(Stream *s, char **args)
{
  ...
  if(CanRemoteCfg.setValue(key, val) == false)
    {
...
bool	cfg_del(Stream *s, char **args)
{
  ...
  if(CanRemoteCfg.delValue(args[2]) == false)
    {
