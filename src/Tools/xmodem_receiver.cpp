...
int XmodemReceiver::init(Stream* src, Stream* dest, size_t size) {
...
int XmodemReceiver::recvSoh() {
    char c = 0;
    const size_t n = CHECK(srcStrm_->readBytes(&c, 1));
    if (n > 0) {
        if (c == Ctrl::CAN) {
...
int XmodemReceiver::recvPacketHeader() {
    packetOffs_ += CHECK(srcStrm_->readBytes(buf_.get() + packetOffs_, sizeof(PacketHeader) - packetOffs_));
...
int XmodemReceiver::recvPacketData() {
    packetOffs_ += CHECK(srcStrm_->readBytes(buf_.get() + packetOffs_, packetSize_ - packetOffs_));
